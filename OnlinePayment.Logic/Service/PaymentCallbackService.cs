
//--------------------------------------------------------------------------------------------------------------------
// Warning! This is an auto generated file. Changes may be overwritten. 
// Generator version: 0.0.1.0
//-------------------------------------------------------------------------------------------------------------------- 

using OnlinePayment.Logic.DataAccess;
using OnlinePayment.Logic.Model;
using Microsoft.Extensions.Logging;

namespace OnlinePayment.Logic.Services
{
    public partial interface IPaymentCallbackService : IService<PaymentCallback>
    {
    }

    public partial class PaymentCallbackService : Service<PaymentCallback>, IPaymentCallbackService
    {
        public PaymentCallbackService(ILogger<PaymentCallbackService> logger,
           IPaymentCallbackDataAccess dataAccess)
           : base(logger, dataAccess)
        { }
    }
}
